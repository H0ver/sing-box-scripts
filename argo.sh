...
installCloudFlared() {
	...
	if [ ${RELEASE[int]} == "CentOS" ]; then
		...
		wget -N https://mirror.ghproxy.com/https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-$cpuArch.rpm
		rpm -i cloudflared-linux-$cpuArch.rpm
		rm -f cloudflared-linux-$cpuArch.rpm
	else
		[ $cpuArch == "aarch64" ] && cpuArch="arm64"
		wget -N https://mirror.ghproxy.com/https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-$cpuArch.deb
...
menu() {
	...
	case "$menuNumberInput" in
		...
		9) wget -N https://raw.githubusercontent.com/H0ver/sing-box-scripts/main/argo.sh && bash argo.sh ;;
		*) exit 1 ;;
	esac
}
